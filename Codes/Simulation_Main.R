...
modelParams<-c('ANCOM', 'CPLM', 'DESeq2', 'edgeR', 'limma', 'limma2', 'limmaVOOM',
               'LM', 'LM2', 'maaslin', 'maaslinNoBoost', 'metagenomeSeq',
               'metagenomeSeq2', 'negbin', 'Spearman', 
...
# Set Additional Options for Methods That Requires TSS Input
TSS_models<-c('maaslin', 'maaslinNoBoost')
if(modelName %in% TSS_models) {
  TSSlist<-list.TSS(simlist.filtered); names(TSSlist) <- names(simlist.filtered)
  fitlist<-TSSlist
}
...
if (!file.exists(outputStringR)){
  ...
  if (modelName=='ANCOM') {
    output<-list.ANCOM(fitlist, transformation=transfMethod)
  }
  ...
  if (modelName=='limma') {
    output<-list.limma(fitlist, transformation=transfMethod)
  }
  
  if (modelName=='limma2') {
    output<-list.limma2(fitlist, transformation=transfMethod)
  }
  
  if (modelName=='limmaVOOM') {
    output<-list.limmaVOOM(fitlist, transformation=transfMethod)
  }
  
  if (modelName=='LM') {
    output<-list.LM(fitlist, transformation=transfMethod)
  }
  
  if (modelName=='LM2') {
    output<-list.LM2(fitlist, transformation=transfMethod)
  }
  
  if (modelName=='maaslin') {
    output<-list.maaslin(fitlist, transformation=transfMethod)
  }
  
  if (modelName=='maaslinNoBoost') {
    output<-list.maaslinNoBoost(fitlist, transformation=transfMethod)
  }
  
  if (modelName=='metagenomeSeq') {
    output<-list.metagenomeSeq(fitlist, transformation=transfMethod)
  }
  
  if (modelName=='metagenomeSeq2') {
    output<-list.metagenomeSeq2(fitlist, transformation=transfMethod)
  }
  ...
  if (modelName=='Spearman') {
    output<-list.Spearman(fitlist, transformation=transfMethod)
  }
  
  if (modelName=='Wilcoxon') {
